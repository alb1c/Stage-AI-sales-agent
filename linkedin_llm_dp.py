...
for employee_dict in employee_dict_list:
    if employee_dict['name'] == 'Davide Morra':
        continue
    if employee_dict_list.index(employee_dict) > 1:#just two messagges
        continue
    ...
    0 - Make the message at most 300 characters.
    1 - Act as the co-founder of your company and present yourself as Davide Morra.
    2 - Start the message with a short information about the customer's company based on the data that is provided you.
    3 - Make it seem like you have a lot of information about the customer's company in the market.
    4 - The goal of the message is to persuade the professional to make a connection.
    5 - Use simple professional language ,
    6 - Avoid using complex words and exaggerated phrases.
    7 - Express interest in collaboration with the customer's company.
    8 - Dont mention specific technologies, focus on the broader concept or benefits.
    9 - Dont express any emotion.
    10 - You are not interested in the role or position the customer has in the company.
    ...
    """}]
        ...
        This message needs refinement to ensure clarity, impact, and engagement.

        Refine this message based on the following guidelines:
    0 - Translate the message in italian.
    1 - Ensure the tone is genuine and authentic, making the message feel original and sincere.
    2 - Always start the message with "Salve" or "Buongiorno".
    3 - Replace any word that conveys emotios with simple words that show professionalism.
    4 - Finish the message with a short greeting.
    5 - Dont show other information in the output besides the message.
    6 - Be careful of syntactical, semantical and grammatical errors.
    7 - If the customer's information is missing, refer to the customer using the name of the company.
    8 - Check if you followed all the guidelines I have provided to you.
    9 - Dont show any other information besides the response.
    """},
        {'role': 'user',
        'content': f"""
    Message to be refined: \{llm_response}
    """}]
                chat_completion = client.chat.completions.create(
                    messages = refinement_context,
                    model=groq_model,
                    max_tokens=32600,
                    temperature=0.5)
