...
employee_dict = get_employees_info()[0]
company_dict = get_companies_info()[0]

print(employee_dict)
print(company_dict)
context = [
    {'role': 'system',
     'content': f""" You are an automated service for an IT company. Your task is to write a cold outbound message to 
     a prospective customer for proposing a partnership between your and theirs companies. The user will provide 
     information about the customer and his/her company, so use them!
...
Ho notato con interesse che Sunnyvale condivide con noi, non solo la sede in Piemonte, ma anche un’attenzione 
particolare alla qualità e una propensione all’innovazione tecnologica che si riflette in un portafoglio clienti di 
alto profilo. La vostra expertise in ambiti quali lo sviluppo, il cloud, le infrastrutture e la cyber security, 
unita alla nostra competenza in soluzioni IT personalizzate, potrebbe tradursi in una sinergia strategica vantaggiosa 
...
---Se ha senso anche per voi, propongo di organizzare una chiacchierata introduttiva dove presentare le nostre 
rispettive realtà ed individuare eventuali sinergie o progetti su cui potremmo collaborare insieme.

Per velocizzare, può trovare qui il mio calendario https://calendly.com/davide-morra/booking. In alternativa, 
se mi fornisce delle date papabili che meglio si adattino alla sua agenda provvedo a mandare quanto prima l’invito. 
--- A presto e buon lavoro, Davide Morra
...
videoconsulti, sistemi di fatturazione specifici per professionisti e pazienti, la definizione di percorsi 
terapeutici tramite calendari ed email transazionali e altre caratteristiche proprie di un sistema ambizioso di 
questo tipo. Per una nota startup simile alla vostra, abbiamo gestito con successo la parte devOps, il backend e 
backoffice e sviluppato sia l’app specialisti (b2b) sia l’app per i clienti finali (b2c) garantendogli un vantaggio 
tecnologico tangibile.
...
---Spero che possa nascere l’interesse per una possibile collaborazione. Nel caso, potremmo organizzare un incontro 
la prossima settimana. Potete scegliere un orario comodo dal mio calendario 
...
enhancement company. you must propose to improve their digital evolution, growth enhancement, helping reaching the 
deadlines, and increase the working areas. This step must follow the instruction provided! This step must be done in 
every message so ALWAYS DO THIS STEP!!!. 
9 - the message should contain only one among these steps: 6, 7 and 8. the 
steps 7, 8 and 9 are mutually exclusive. 
9 - you must include a paraphrasis of the parts in diamond brackets!! the 
paraphrasis must include all the links provided in the bullet list. 
10 - you must include a paraphrasis of the parts 
between triple dashes and always use the link!! 
11 - if the customer's information is missing, refer to the customer 
using the name of the company. 
12 - if you wrote more paragraphs about the same topic, just pick one and delete the 
other. 
13 - if the message contains triple dashes, YOU MUST delete them. 
14 - if the message contains diamond 
brackets, YOU MUST  delete them. 
15 - if the message contains square brackets, YOU MUST delete them. 
16 - check if 
you missed anything on previous steps. 
17 - if the message does not contain a paragraph about the customer, 
add the paragraph to the message. 
18 - check if you followed all the guidelines I have provided to you.
19 - check again if the message is entirely in italian.
20 - remove every data you used to write the message.
